        ...
        mClient.newCall(client.buildRequest()).enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                if (response.isSuccessful()) {
                    ...
                    try {
                        if (response.request().url().toString().indexOf("https://open.douyin.com/oauth/access_token/") != -1) {
                            //抖音登录拦截下来
                            String msg = JSONUtils.getNoteJson(result, "message");
...
//                                ToastUtils.show(activity,msg+"");
                            ...
                            MyApplication.mPreferenceProvider.setLastTime(JSONUtils
                                    .getNoteJson(user, "user_vip_last_time"));
                        }
                    } catch (Exception e) {
//                        activity.runOnUiThread(() -> ToastUtils.show(activity, "服务器异常，请稍后再试"));
