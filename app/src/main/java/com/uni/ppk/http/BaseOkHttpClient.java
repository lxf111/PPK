...
public class BaseOkHttpClient {
    ...
    public Request buildRequest() {
        ...
        builder.addHeader("user-token", "" + (MyApplication.mPreferenceProvider.getToken() == null ?
                "" : MyApplication.mPreferenceProvider.getToken()));
        builder.addHeader("way", "user");
...
//                .addHeader("Vary", "Accept-Encoding");
    ...
    private String buildGetRequestParam() {
        ...
        if (mBuilder.isParams) {
            //map形式
            String json = JSONUtils.toJsonString(mBuilder.parameter.getParams());
//            builder.appendQueryParameter(json);
            Set<Map.Entry<String, Object>> set = mBuilder.parameter.getParams().entrySet();
            // 遍历键值对对象的集合，得到每一个键值对对象
            for (Map.Entry<String, Object> me : set) {
                String key = me.getKey();
                Object value = me.getValue();
                builder.appendQueryParameter(key, "" + (value == null ? "" : value));
            }
            String url = builder.build().toString();
            LogUtils.e("TAG", "************get---参数 =" + json);//application/json; charset=utf-8
            LogUtils.e("TAG", "************get---url =" + url);//application/json; charset=utf-8
            return url;
        }
...
//            return RequestBody.create(MediaType.parse("multipart/form-data"), json);
            ...
            Object[] strs = mBuilder.parameter.getParams().keySet().toArray();
            MultipartBody.Builder builder = new MultipartBody.Builder();
            builder.setType(MultipartBody.FORM);
            JSONObject jsonObj = new JSONObject();
            for (Object a : strs) {
                String value = String.valueOf(mBuilder.parameter.getParams().get(a));
                jsonObj.put(a.toString(), value == null ? "" : value);
                builder.addFormDataPart(a.toString(), value == null ? "" : value);
            }
        ...
        for (RequestParameter p : mBuilder.params) {
            if (p.getFile() != null) {
                ...
                builder.addFormDataPart(p.getKey(), p.getObj() == null ?
                        "" : p.getObj().toString(), RequestBody.create(type, p.getFile()));
            } else {
    ...
    public static class Builder {
        ...
        public Builder params(Map<String, Object> param) {
            parameter = new RequestParameter(param);
            isParams = true;
            return this;
        }
